...
function Connection(connectionOptions, options) {
    ...
    this.connectionOptions  = extend({}, connectionOptions);
...
Connection.prototype.handleMethod = function handleMethod(
    ...
    default:
        this.emit("error", {
            message: "Unhandled method: " + method.name + " with args " + stringify(args)
        });
    }
...
Connection.prototype.handleConnStart = function handleConnStart(args) {
    ...
    if (args.versionMajor !== 0 && args.versionMinor !== 9) {
        this.end();
        this.emit("error", { message: "Bad server version" });
        return;
    }
